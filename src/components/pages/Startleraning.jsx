...
import React, { useEffect, useState } from "react";
import { useParams, Link, useNavigate } from "react-router-dom";
import { collection, getDocs, doc, getDoc, updateDoc, onSnapshot, query, orderBy, limit,where } from "firebase/firestore";
import { PieChart } from "react-minimal-pie-chart";
import { getAuth, onAuthStateChanged } from "firebase/auth";
import { db } from "../../firebaseConfig";
import QandASection from "./QuestionAndAns"; // Import the Q&A section component
  ...
  useEffect(() => {
    const fetchEMIDetails = async () => {
      ...
      try {
        ...
        const q = query(paymentsRef, where("userId", "==", userEmail));
        ...
        for (const paymentDoc of querySnapshot.docs) {
          ...
          if (emiPlanSnap.exists()) {
            ...
            const paidEMIs = await getDocs(query(
            ...
            if (nextEMINumber <= totalEMIs) {
              ...
              const lastPaymentDate = paymentData.timestamp?.toDate() || new Date();
